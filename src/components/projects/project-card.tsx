...
export function ProjectCard({ project }: { project: Project }) {
  ...
  return (
    <div className="group/lift relative block rounded-2xl" data-animate>
      <article className="glass-card group flex h-104 flex-col overflow-hidden rounded-2xl p-1 transition-[transform,box-shadow] duration-300 ease-out group-hover/lift:-translate-y-1 group-hover/lift:shadow-xl group-hover/lift:shadow-primary-500/10">
        {/* Thumbnail */}
        ...
        {/* Content — relative so the pinned bar can position inside */}
        <div className="relative flex flex-1 flex-col overflow-hidden px-4 py-3">
          ...
          {project.tagline && (
            <p className="mt-1 line-clamp-1 text-xs font-medium text-primary-500">
              {project.tagline}
            </p>
          )}
          ...
          {/* Action links — pinned to bottom with gradient fade */}
          <div className="absolute inset-x-0 bottom-0 px-4 pb-3">
            <div className="pointer-events-none absolute inset-x-0 -top-6 h-6 bg-linear-to-t from-background/90 to-transparent" />
            <div className="relative flex items-center gap-3 bg-background/90 pt-1.5 backdrop-blur-xs">
