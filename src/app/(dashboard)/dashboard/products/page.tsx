...
export default function ProductsPage() {
  ...
  const columns: Column<AdminProduct>[] = [
    ...
    },
    {
      ...
      sortable: true,
      render: (r) =>
        `${r.currency} ${Number(r.price).toLocaleString()}${r.salePrice ? ` (${r.currency} ${Number(r.salePrice).toLocaleString()})` : ""}`,
    },
    { key: "licenseType", label: "License" },
    {
      ...
      label: "Featured",
      render: (r) => (
        <span className={r.isFeatured ? "text-primary-500" : "text-muted-foreground"}>
          {r.isFeatured ? "Yes" : "No"}
        </span>
      ),
    },
