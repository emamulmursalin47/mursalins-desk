...
export default function TestimonialsPage() {
  ...
  const columns: Column<AdminTestimonial>[] = [
    ...
    { key: "createdAt", label: "Date", sortable: true, render: (r) => new Date(r.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) },
    {
      ...
      render: (r) => (
        <div className="flex items-center gap-2">
          <button onClick={(e) => { e.stopPropagation(); router.push(`/dashboard/testimonials/${r.id}/edit`); }} className="cursor-pointer text-xs font-medium text-primary-600 hover:text-primary-500">Edit</button>
  ...
  return (
    <div ref={containerRef}>
      ...
      {loading ? <LoadingState /> : (
        <div data-animate>
          <DataTable columns={columns} data={filtered} keyExtractor={(r) => r.id} onRowClick={(r) => router.push(`/dashboard/testimonials/${r.id}/edit`)} emptyMessage="No testimonials found." />
